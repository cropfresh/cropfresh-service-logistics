...
const drop_point_assignment_service_1 = require("../../src/services/drop-point-assignment-service");
// Mock Prisma
...
const mockDropPoints = [
    {
        ...
        crateInventory: { '20kg': 50, '50kg': 30 },
        operatingHours: { mon: '06:00-18:00', tue: '06:00-18:00' },
        createdAt: new Date(),
        updatedAt: new Date(),
    },
    {
        ...
        isActive: true,
        maxDailyCapacity: 50,
        crateInventory: { '20kg': 20, '50kg': 10 },
        operatingHours: { mon: '07:00-17:00', tue: '07:00-17:00' },
        createdAt: new Date(),
        updatedAt: new Date(),
    },
];
...
describe('DropPointAssignmentService', () => {
    ...
    beforeEach(() => {
        ...
        service = new drop_point_assignment_service_1.DropPointAssignmentService();
    });
    // --------------------------------------------------------------------------
    // assignDropPoint
    // --------------------------------------------------------------------------
    describe('assignDropPoint', () => {
        it('should assign nearest drop point with capacity (AC2)', async () => {
            // Arrange
            prismaMock.dropPoint.findMany.mockResolvedValue(mockDropPoints);
        ...
        it('should throw error when no drop points available', async () => {
            ...
            // Act & Assert
            await expect(service.assignDropPoint({
                listingId: 1,
                farmerId: 1,
                farmerLocation: mockFarmerLocation,
                cropType: 'Tomatoes',
    ...
    describe('reassignDropPoint', () => {
        it('should update assignment to new drop point (AC6)', async () => {
            ...
            prismaMock.dropPoint.findUnique.mockResolvedValue(mockDropPoints[1]);
        ...
        it('should throw error for non-existent assignment', async () => {
            ...
            prismaMock.dropPointAssignment.findFirst.mockResolvedValue(null);
            // Act & Assert
            await expect(service.reassignDropPoint(999, 'dp-002', 'Test')).rejects.toThrow();
        });
    });
});
